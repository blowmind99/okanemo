...
@push('title', 'Edit')
@push('subtitle', 'Edit')
@push('description', 'Edit course')
@push('tools')
    @include('layout.public-component.back-button', ['url' => url('course')])
@endpush
@section('content')
<div class="nk-block nk-block-lg">
    <form action="{{ url('course/'.$course->id) }}" method="post" class="form-input">
        @csrf
        @method('put')
                    ...
                    <div class="card-inner">
                        <div class="mb-4">
                            ...
                            <div class="form-control-wrap">
                                <input type="text" name="name" class="form-control @error('name') error @enderror" value="{{ old('name', $course->name) }}" required>
                            ...
                            <div class="form-control-wrap @error('category_id') border border-danger rounded @enderror">
                                <select name="category_id" class="form-control js-select2" required>
                                    @foreach ($categories as $category)
                                        <option value="{{$category->id}}" {{ (old('category_id', $course->category->id) == $category->id) ? 'selected' : '' }}>{{ $category->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <div class="mb-4">
                            ...
                            <div class="form-control-wrap @error('type') border border-danger rounded @enderror">
                                <select name="type" class="form-select js-select2">
                                    <option value="self" {{old('type', $course->type) == 'self' ? 'selected' : ''}}>Self learning</option>
                                    <option value="online" {{old('type', $course->type) == 'online' ? 'selected' : ''}}>Online learning</option>
                                    <option value="offline" {{old('type', $course->type) == 'offline' ? 'selected' : ''}}>Offline learning</option>
                                </select>
                            </div>
                        ...
                        <div class="mb-4">
                            <label class="form-label required">Type</label>
                            <div class="form-control-wrap @error('status') border border-danger rounded @enderror">
                                <select name="status" class="form-select js-select2">
                                    <option value="active" {{old('status', $course->status) == 'active' ? 'selected' : ''}}>Active</option>
                                    <option value="inactive" {{old('status', $course->status) == 'inactive' ? 'selected' : ''}}>Inactive</option>
                                </select>
                            </div>
                            <span class="d-flex justify-content-end mt-1 letter-space-1 fs-12px"> course status </span>
                            @include('layout.public-component.error-input', ['name' => 'status'])
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <div class="form-group">
                                <div class="form-control-wrap">
                                    <textarea name="description" class="form-control no-resize @error('description') error @enderror" id="default-textarea">{{ old('description', $course->description) }}</textarea>
                                </div>
                            </div>
