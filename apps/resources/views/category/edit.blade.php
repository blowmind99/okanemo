...
@push('title', 'Edit')
@push('subtitle', 'Edit')
@push('description', 'Edit category')
@push('tools')
    @include('layout.public-component.back-button', ['url' => url('category')])
@endpush
@section('content')
<div class="nk-block nk-block-lg">
    <form action="{{ url('category/'.$category->id) }}" method="post" class="form-input">
        @csrf
        @method('put')
                    ...
                    <div class="card-inner">
                        <div class="mb-4">
                            ...
                            <div class="form-control-wrap">
                                <input type="text" name="name" class="form-control @error('name') error @enderror" value="{{ old('name', $category->name) }}" required>
